...
You should see two environments in `Inspectify`:
...
You will code code both the `BiGCL` transpiler and the `RiscV` compiler.
...
1. The languages use different syntax. For example, consider how computing the sum of two numbers is done in each language:
    ...
    * RISC-V:
        ...
        add a, b, c
        ```
    In addition, GCL can operate directly with variables, while in RISC-V arithmetic instructions use registers and dedicated instructions are used to load/store between registers and memory. 
...
There is something wrong tough... you will have to discover what.
...
The input is a GCL program. The output should be an equivalnet RISC-V program.
...
4. Observe the obtained graph.
    ...
    * Would you be able translate them into RISC-V code?
        <details>
        <summary>Answer</summary> single assignmetns reselble load/store RISC-V operations `li`, `lw`,...., and assignments with 2 operands resemble RISC-V operations `add`, `sub`,... 
        </details>

> Your generated RISC-V code is allowd to use the operations listed in the [lecture notes](https://courses.compute.dtu.dk/02247/f26/risc-v.html), except for the floating point operations. If you need more instruction... ask the teacher :)
...
* Go through all edges and translate their labels into the corresponding RISC-V code. Some cases will be trivial, some may require more though.
