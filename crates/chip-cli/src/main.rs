    ...
    match &cli.cmd {
        Cmd::ChipCheck {
            ...
            for e in std::fs::read_dir(&reference)? {
                ...
                let path = Utf8PathBuf::from_path_buf(e.path().to_path_buf()).map_err(|p| {
                    color_eyre::Report::msg(format!("could not convert path: {:?}", p))
                })?;
            ...
            for (g_idx, g) in groups.groups.iter().enumerate() {
                ...
                for (t_idx, t) in tests.iter().enumerate() {
                    ...
                    for assertion in p.assertions() {
                        let backend =
                            smtlib::backend::z3_binary::tokio::Z3BinaryTokio::new("z3").await?;
